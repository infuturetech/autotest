...
@pytest.mark.p2
def test_create_two_algo_task_use_same_camera_and_same_app_and_same_decoder(host):
    """
    同一条相机使用相同的解码配置同时下发2路同一算法的推理任务
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 两条相机使用相同的解码配置同时下发同一算法的推理任务")
    ...
    rr3 = OpenApi.create_algo_task(host, algo_id, camera_id, decode_config=decoder_cfg)
