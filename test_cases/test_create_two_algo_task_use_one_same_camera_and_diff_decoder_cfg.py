...
@pytest.mark.p1
def test_create_two_algo_task_use_one_same_camera_and_diff_decoder_cfg(host):
    """
    同一相机使用不同的解码配置同时成功下发两路推理任务
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 同一相机使用不同的解码配置同时成功下发两路推理任务")
    ...
    rr3 = OpenApi.create_algo_task(host, algo_id, camera_id, decode_config=decoder_cfg2)
