...
@pytest.mark.p1
def test_create_two_algo_task_use_diff_camera_and_same_app(host):
    """
    两条相机使用不同的解码配置同时下发同一算法的推理任务
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 两条相机使用不同的解码配置同时下发同一算法的推理任务")
    ...
    rr3 = OpenApi.create_algo_task(host, algo_id, camera_id2, decode_config=decoder_cfg2)
