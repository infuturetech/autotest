...
@pytest.mark.p1
def test_delete_algo_results_use_invalid_time_range(host):
    """
    删除指定算法某个不存在推理结果时间段的数据
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 删除指定算法某个不存在推理结果时间段的数据")
    ...
    rt = OpenApi.delete_algo_task_result_time_range(host, algo_id, start_time, end_time, stream_id)
