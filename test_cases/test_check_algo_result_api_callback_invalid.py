...
@pytest.mark.p2
def test_check_algo_result_api_callback_invalid(host):
    """
    设置错误的回调接口验证推理结果进行接口回调
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 设置错误的回调接口验证推理结果进行接口回调")
    ...
    OpenApi.set_call_back(host, stream_id, mock_callback_api + "/test")
