...
from aws.common import upload_app_file_to_server, do_recv_algo_result_call_back
...
@pytest.mark.p2
def test_check_algo_result_api_callback_streamid_invalid(host):
    """
    设置错误的流id验证推理结果进行接口回调
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 设置错误的流id验证推理结果进行接口回调")
    ...
    log.info("启动回调监听")
    do_recv_algo_result_call_back(host)
    ...
    OpenApi.set_call_back(host, stream_id + "_test", mock_callback_api)

    time.sleep(20)

    algo_results = do_recv_algo_result_call_back(host, "stop")  

    log.info(f"获取到回调输出的部分推理结果: {algo_results}")
    ...
    assert stream_id not in str(algo_results)
