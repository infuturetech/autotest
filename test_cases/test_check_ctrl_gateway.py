...
@pytest.mark.p0
def test_check_ctrl_gateway(host):
    """
    检查gateway服务启动停止
    Args:
        host (_type_): _description_
    """
    log.info("测试点：检查gateway服务启动停止")
    ctrl_service(host, GATEWAY, "stop")
    time.sleep(5)
    ctrl_service(host, GATEWAY, "start")
    time.sleep(8)    
    assert check_service_is_running(host, GATEWAY)
