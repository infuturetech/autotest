...
@pytest.mark.p2
def test_delete_app_data_one_create_algo_task(host):
    """
    app已下发推理任务进行app包删除
    Args:
        host (_type_): _description_
    """
    log.info("测试点: app已下发推理任务进行删除")
    ...
    del_app = OpenApi.delete_app_packet(host, algo_id)

    ret = OpenApi.delete_camera(host, camera_id)


    
    assert del_app["result"]["code"] == 0
