...
@pytest.mark.p0
def test_create_two_algo_task_use_diff_app(host):
    """
    两条相机分别使用不同的算法下发推理任务
    Args:
        host (_type_): _description_
    """
    log.info("测试点: 两条相机分别使用不同的算法下发推理任务")
    ...
    rr2 = OpenApi.upload_app_packet(host, local_path, algo_type=1, algo_name="人脸检测demo", algo_version="v1.0",
                                    describe="测试")
    algo_id2 = rr["data"]["algo_id"]
